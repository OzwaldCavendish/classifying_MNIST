...
As we can see in the figure above [DN: Add a figure above] the classes 0-9 for MNIST are roughly balanced - in this case "Accuracy" and "Area Under Curve" are suitable metrics for model performance.  If the dataset were unbalanced, "Precision", "Recall" and their average "F1-Score" would be good choices because they can penalise the model for poor performance on important (for humans) minority classes.
