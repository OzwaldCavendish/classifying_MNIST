...
In the interest of honesty I found an introduction to Keras for R at https://blog.rstudio.com/2017/09/05/keras-for-r/, and both this introduction and the book Deep Learning in Python by F. Chollet use MNIST heavily as a teaching example.  I've tried to forget what I learned of specific parameters in building these NN's, and in fairness the guides did not use PCA, but I should own up to having had the help.

PS; Yes, Keras for R is just calling Python in the background.  Yes, that's just embarassing for R.  But ONS insists...
...
The two candidate models, k-NN (k=3) and the best-performing neural network (two hidden layers of 256 and 32 neurons, dropout rate of 0.2) were chosen based upon their cross-validated accuracies.  the kNN algorithm manages as accuracy of 0.9743, while the neural network improves on this with 0.9804, a relative gain of 24 %.  An important property beside accuracy is whether the model performs equally well across all classes.  The final confusion matrix indicates that this is the case, meaning the model would not have a bias problem if used "in anger".  An added practical advantage of the neural network is that it is much, much faster to train and apply than the kNN algorithm.
...
The neural network outperforms kNN, but only just.  The biggest advantage to the neural network is that it scales well to larger datasets in terms of computational time.  On this large-ish dataset the neural network already runs significantly faster than kNN - any more training data would make kNN unuseably slow - at least with this implementation, which appears to use only one core on my laptop.  Comparing the best performance here (accuracy of NN = 0.9804 on test data) to examples found on the MNIST database, it doesn't fare badly.  kNN has been used to achieve much better results than the PCA + k=3 implementation through the use of non-linear deformations to transform the feature space (test error rate of 0.52, [DN:  Add ref!]).  Convolutional neural networks, and neural networks in general with more neurons and more sophisticated loss functions, consistently achieve < 1 % test error rates (eg; [DN: Add ref!]).
