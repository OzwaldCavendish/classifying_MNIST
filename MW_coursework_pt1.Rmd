...
The two candidate models, k-NN (k=3) and the best-performing neural network (two hidden layers of 256 and 32 neurons, dropout rate of 0.2) were chosen based upon their cross-validated accuracies.  the kNN algorithm manages as accuracy of 0.9743, while the neural network improves on this with 0.9827, a relative gain of 67 %.  An important property beside accuracy is whether the model performs equally well across all classes.  The final confusion matrix indicates that this is the case, meaning the model would not have a bias problem if used "in anger".  An added practical advantage of the neural network is that it is much, much faster to train and apply than the kNN algorithm.
